...
# 为导入的 filter 模块指定别名，以避免与 Python 内置的 filter() 函数产生命名冲突。
from astrbot.api.event import filter as event_filter, AstrMessageEvent
from astrbot.api import logger, AstrBotConfig

# --- Data Models ---
# 定义清晰、类型安全的数据结构，作为各逻辑模块之间的契约。
...
class MetricsFormatter:
    ...
    def format(self, metrics: SystemMetrics) -> str:
        ...
        parts = [
            "💻 **服务器实时状态**", "--------------------",
            self._format_uptime(metrics.uptime), self._format_cpu(metrics),
